...
class BronzeWareDataset1(Dataset):#给随机分配数据集（名字）关联上对应图片的函数
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.from_numpy(np.asarray([self.ware_shape[idx]]).astype('float64'))


class BronzeWareDataset2(Dataset):#给随机分配数据集（名字）关联上对应图片的函数
    def __init__(self, root_dir, ware_data, transform=None):
        self.root_dir = root_dir
        self.ware_img_name = ware_data[:, 0]
        self.ware_age = ware_data[:, 1]
...
class BronzeWareDataset3(Dataset):#给随机分配数据集（名字）关联上对应图片的函数
    def __init__(self, root_dir, ware_data, transform=None):
        self.root_dir = root_dir
        self.ware_img_name = ware_data[:, 0]
        self.ware_age = ware_data[:, 1]
