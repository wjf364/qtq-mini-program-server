...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        # 计算预测的目标分数（这里的目标只是指前景或者背景），预测anchor的目标分数，num_anchors来自于参数设定，下同
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 计算预测的目标bbox regression参数，每个anchors会预测四个回归偏移量，所以乘以4
    ...
    def forward(self, x):#x就是之前的feature
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head,
        ...
        self.anchor_generator = anchor_generator#生成anchor
        self.head = head#头的部分
        self.box_coder = det_utils.BoxCoder(weights=(1.0, 1.0, 1.0, 1.0))

        # use during training
        # 计算anchors与真实bbox的iou
        self.box_similarity = box_ops.box_iou#计算传入两组iou的值，下方的函数，赋值的是方法

        self.proposal_matcher = det_utils.Matcher(#管理正负样本
    ...
    def forward(self,
        ...
        objectness, pred_bbox_deltas = self.head(features)#这两个是rpn中的两个1*1的分支（损失值、回归参数）
        ...
        proposals = self.box_coder.decode(pred_bbox_deltas.detach(), anchors)#decoder方法是另外的函数
