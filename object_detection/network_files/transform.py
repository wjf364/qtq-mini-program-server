...
#transfrom主要函数，可以reszie,也可以resize回
class GeneralizedRCNNTransform(nn.Module):
    ...
    def normalize(self, image):
        ...
        return (image - mean[:, None, None]) / std[:, None, None]#减去均值，再除以方差，就达成目的
    ...
    def resize(self, image, target):
        ...
        h, w = image.shape[-2:]#先获取传入图像的信息（高度和宽度）
        ...
        #对图像进行resize，下面的两个函数在上方引用了
        if torchvision._is_tracing():
            image = _resize_image_onnx(image, size, float(self.max_size))
        else:
            image = _resize_image(image, size, float(self.max_size))

        if target is None:#训练模式，直接返回就行，毕竟没有target
            return image, target

        bbox = target["boxes"]#测试模式，有target,也就是有bbox，所以要对bbox进行处理
        # 根据图像的缩放比例来缩放bbox
        bbox = resize_boxes(bbox, [h, w], image.shape[-2:])#bbox也resize了
        target["boxes"] = bbox
    ...
    #这里的batch是自己定义的，datast里面，一个batch里面的图片的尺寸都是相同的，应该是所有的图片的batch都是相同的
    def batch_images(self, images, size_divisible=32):#将resize后的图片打包一个batch返回，保持了原始图像的比例，减少对图像检测的干扰
        # type: (List[Tensor], int) -> Tensor
        """
    ...
    #将预测图片还原原来尺度
    #下面传入三个参数，result对应网络预测的结果，包含有边界框、坐标信息、标签值以及概率，第二个参数对应缩放后的尺度、第三个就是原图像的尺寸
    ...
    def forward(self,
        ...
        images = [img for img in images]#判断输入的每一张图片
        for i in range(len(images)):
            image = images[i]
            target_index = targets[i] if targets is not None else None#判断是训练的还是测试的

            if image.dim() != 3:#看输入的图像是否是RGB图像，这个位置我可能需要改一下，因为有灰度图片
