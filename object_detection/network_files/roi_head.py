...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # 计算类别损失信息，这个就是交叉熵
    #注意，这里面所有的label都应该是一个数，我要把这些书concat起来，然后和一个label做交叉熵，或者是只取前五个class_logits，加上原来的feature，就形成了我想要的交叉熵
    #现在要知道这些class_logits是不是按照大小顺序拍的，或者格式如何，我需要找出前五个最大的
    ...
    sampled_pos_inds_subset = torch.where(torch.gt(labels, 0))[0]#torch.where，条件限制，这里用来返回索引不是0的数
...
class RoIHeads(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.box_similarity = box_ops.box_iou#赋值了IOU方法
        ...
        self.fg_bg_sampler = det_utils.BalancedPositiveNegativeSampler(#对划分好的正负样本进行采样，正样本25%
    ...
    #传入当前图片的全局特征，当前图片的索引框，原本图片的形状以及是否有的target
    '''
    重要问题，当前传入的features是代表一张图片的还是一个batchsize大小的图片的，这很重要
    '''
    def forward(self,
        ...
        box_features = self.box_head(box_features)#TO MLPhead

        # 接着分别预测目标类别和边界框回归参数
        class_logits, box_regression = self.box_predictor(box_features)#算出了损失，我的值罢了

        result = torch.jit.annotate(List[Dict[str, torch.Tensor]], [])#分别是1024，21和1024，84，要改到这里
        losses = {}#训练模式的画就存储起来
        if self.training:
