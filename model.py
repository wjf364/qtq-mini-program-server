...
def test(image_path,model):
    ...
    #预测出两个最大值为商晚和西早，则将该年代定义为商晚或西早，其余正常
    a = predicted2[0][0]
    b = predicted2[0][1]
    a1 = float(a)
    b1 = float(b)
    ...
    if a == 1 and b == 2:
        ...
        for key, values in age_idx.items():  # 同时便利键值对
            predicted = float(predicted)  # 后面带小数点不能直接转成int,需要进行下面的操作才行
...
def classification(image_path):
    ...
    trained_weight = torch.load("final_net.pth")
    net.load_state_dict(trained_weight.state_dict())
    return test(image_path, net)
