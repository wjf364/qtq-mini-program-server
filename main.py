...
from model_det import object_detection
...
def uploader():
    if request.method == 'POST' and 'file' in request.files:
        ...
        # 进行图片处理
        # 分类网络.....
        class_label, class_score = classification(img_path)
        # 目标检测网络.....
        prediction, labels, _ = object_detection(img_path)
        dict = {}
        dict['status'] = 200
        dict['msg'] = '检测到物体'
        dict['prediction'] = class_label
        # dict['prediction'] = label
        dict['score'] = class_score
        # dict['score'] = prediction['scores']
        if len(labels) == 0:
            dict['bbox'] = []
        else:
            dict['bbox'] = prediction['boxes']
        #     return jsonify(status = 201, msg = '没有检测到物体')
        print(jsonify(dict))
        return jsonify(dict)
